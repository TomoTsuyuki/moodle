...
class auth_test extends \advanced_testcase {
    ...
    public function test_oauth2_complete_login(): void {
        ...
        $event1 = $events[0];
        $event2 = $events[1];
        $this->assertInstanceOf('core\event\user_updated', $event1);
        $this->assertInstanceOf('core\event\user_loggedin', $event2);

        // Make sure the extra record is in the event.
        $idpdata = $event2->other['idp_data'];
        $this->assertEquals($info, $idpdata);
    }
}
