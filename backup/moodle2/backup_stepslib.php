    ...
    protected function define_structure() {
        ...
        $role->set_source_sql_using_backup_ids("
         ...
         LEFT JOIN {role_names} rn ON r.id = rn.roleid AND rn.contextid = ?
             WHERE r.id *SQL*", [backup::VAR_CONTEXTID], backup::VAR_BACKUPID, 'rolefinal');
...
class backup_final_scales_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $scale->set_source_sql_using_backup_ids("
            SELECT s.*
              FROM {scale} s
             WHERE s.id *SQL*", [], backup::VAR_BACKUPID, 'scalefinal');
    ...
    protected function define_structure() {
        ...
        $outcome->set_source_sql_using_backup_ids("
            SELECT o.*
             ...
             WHERE o.id *SQL*", [], backup::VAR_BACKUPID, 'outcomefinal');
    ...
    protected function define_structure() {
        ...
        if ($groupinfo) {
            $group->set_source_sql_using_backup_ids("
                SELECT g.*
                  FROM {groups} g
                 WHERE g.id *SQL*", [], backup::VAR_BACKUPID, 'groupfinal');

            $grouping->set_source_sql_using_backup_ids("
                SELECT g.*
                  FROM {groupings} g
                 WHERE g.id *SQL*", [], backup::VAR_BACKUPID, 'groupingfinal');
    ...
    protected function define_structure() {
        ...
        $user->set_source_sql_using_backup_ids('
         ...
         LEFT JOIN {mnet_host} m ON m.id = u.mnethostid
             WHERE u.id *SQL*', [], $this->get_backupid(), 'userfinal');
    ...
    protected function define_structure() {
        ...
        $file->set_source_sql_using_backup_ids("
         ...
         LEFT JOIN {repository} r ON r.id = ri.typeid
             WHERE f.id *SQL*", [], backup::VAR_BACKUPID, 'filefinal');
...
class backup_annotate_groups_from_groupings extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        $groupings = backup_muc_manager::get_allkeys($this->get_backupid(), 'grouping');
        if ($groupings) {
...
class backup_annotate_scales_from_outcomes extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        $outcomes = backup_muc_manager::get_allkeys($this->get_backupid(), 'outcome');
        if ($outcomes) {
...
class backup_annotate_all_question_files extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        $content = backup_muc_manager::get_allkeys($this->get_backupid(), 'question_categoryfinal');
        if (empty($content)) {
            return;
        }
    ...
    protected function define_structure() {
        ...
        $qcategory->set_source_sql_using_backup_ids("
            SELECT gc.*, contextlevel, instanceid AS contextinstanceid
             ...
             WHERE gc.id *SQL*", [], backup::VAR_BACKUPID, 'question_categoryfinal');
...
class backup_annotate_all_user_files extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        $users = backup_muc_manager::get_allkeys($this->get_backupid(), 'userfinal');
        $progress = $this->task->get_progress();
...
class backup_activity_grades_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $item->set_source_sql_using_backup_ids("
            SELECT gi.*
              FROM {grade_items} gi
             WHERE gi.id *SQL*", [], backup::VAR_BACKUPID, 'grade_item');
    ...
    protected function define_structure() {
        ...
        if ($userinfo && $history) {
            ...
            $grade->set_source_sql_using_backup_ids("
                SELECT ggh.*
                  FROM {grade_grades_history} ggh
                 WHERE ggh.id *SQL*", [], backup::VAR_BACKUPID, 'grade_item');
