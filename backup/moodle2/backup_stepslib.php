    ...
    protected function define_structure() {
        ...
        $role->set_source_sql_using_backup_ids("
         ...
         LEFT JOIN {role_names} rn ON r.id = rn.roleid AND rn.contextid = ?
             WHERE r.id *SQL*", array(backup::VAR_CONTEXTID),  backup::VAR_BACKUPID, 'rolefinal', 'itemid');
...
class backup_final_scales_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $scale->set_source_sql_using_backup_ids("
            SELECT s.*
              FROM {scale} s
             WHERE s.id *SQL*", array(), backup::VAR_BACKUPID, 'scalefinal', 'itemid');
    ...
    protected function define_structure() {
        ...
        $outcome->set_source_sql_using_backup_ids("
            SELECT o.*
             ...
             WHERE o.id ", array(), backup::VAR_BACKUPID, 'outcomefinal', 'itemid');
    ...
    protected function define_structure() {
        ...
        if ($groupinfo) {
            $group->set_source_sql_using_backup_ids("
                SELECT g.*
                  FROM {groups} g
                 WHERE g.id *SQL*", array(), backup::VAR_BACKUPID, 'groupfinal', 'itemid');
            ...
            $grouping->set_source_sql_using_backup_ids("
                SELECT g.*
                  FROM {groupings} g
                 WHERE g.id *SQL*", array(), backup::VAR_BACKUPID, 'groupingfinal', 'itemid');
    ...
    protected function define_structure() {
        ...
        $file->set_source_sql_using_backup_ids("
         ...
         LEFT JOIN {repository} r ON r.id = ri.typeid
             WHERE f.id *SQL*", array(), backup::VAR_BACKUPID, 'filefinal', 'itemid');
...
class backup_annotate_groups_from_groupings extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        foreach ($groupings as $grouping) {
            if ($groups = $DB->get_records('groupings_groups', array('groupingid' => $grouping))) {
                foreach ($groups as $group) {
...
class backup_annotate_scales_from_outcomes extends backup_execution_step {
    ...
    protected function define_execution() {
        ...
        foreach ($outcomes as $outcome) {
            if ($scale = $DB->get_record('grade_outcomes', array('id' => $outcome))) {
    ...
    protected function define_structure() {
        ...
        $qcategory->set_source_sql_using_backup_ids("
            SELECT gc.*, contextlevel, instanceid AS contextinstanceid
             ...
             WHERE gc.id *SQL*", array(), backup::VAR_BACKUPID, 'question_categoryfinal', 'itemid');
...
class backup_activity_grades_structure_step extends backup_structure_step {
    ...
    protected function define_structure() {
        ...
        $item->set_source_sql_using_backup_ids("
            SELECT gi.*
              FROM {grade_items} gi
             WHERE gi.id *SQL*", array(), backup::VAR_BACKUPID, 'grade_item', 'itemid');
    ...
    protected function define_structure() {
        ...
        if ($userinfo && $history) {
            ...
            $grade->set_source_sql_using_backup_ids("
                SELECT ggh.*
                  FROM {grade_grades_history} ggh
                 WHERE ggh.id *SQL*", array(), backup::VAR_BACKUPID, 'grade_item', 'itemid');
