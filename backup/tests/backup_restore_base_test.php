 ...
 */
class core_backup_backup_restore_base_testcase extends advanced_testcase {
    ...
    public function test_having_settings(): void {
        $this->assertEquals(1, get_config('backup', 'backup_general_permissions'));
        $this->assertEquals(0, get_config('backup', 'backup_import_permissions'));
        $this->assertEquals(1, get_config('backup', 'backup_auto_permissions'));
    ...
    /**
     * Backup the course by auto mode.
     *
     * @param  stdClass $course Course for backup.
     * @return string Hash string ID from the backup.
     * @throws coding_exception
     * @throws moodle_exception
     */
    protected function perform_auto_backup($course): string {
        global $CFG, $USER;
        ...
        // Get the backup file.
        $fs = get_file_storage();
        $files = $fs->get_area_files($coursecontext->id, 'backup', 'automated', false, 'id ASC');
        $backupfile = reset($files);
