...
abstract class backup_question_dbops extends backup_dbops {
    ...
    public static function calculate_question_categories($backupid, $contextid) {
        ...
        $rs = $DB->get_recordset_sql("SELECT id FROM {question_categories} WHERE contextid = ?", array($contextid));
        $cacheqc = backup_muc_manager::get('question_category');
        foreach ($rs as $record) {
            $cacheqc->set($record->id, 1);
        }
        $rs->close();
        ...
        $cacheq = backup_muc_manager::get('question');
